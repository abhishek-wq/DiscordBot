...
os.chdir('C:/Users/relience/Desktop/MyBot')
...
async def on_message(message):
    if client.user == message.author:
        return
    if message.channel.id == 844113530638172211 or message.channel.id == 844116243044630529 or message.channel.id == 844118529279000587:
...
@client.event
async def on_member_join(member):
    embed = discord.Embed(title = member.name, description = f"{member.mention} has joined", color = discord.Colour.blue())
    embed.add_field(name = "ID",value =member.id, inline = True )
    await client.get_channel().send(embed = embed)
...
async def on_member_remove(member):
    await client.get_channel().send(f"{member.name} has left.")
...
@client.command(aliases=['remove','out','k'])
...
@client.command(aliases=['b','permanent-remove'])
...
@client.command(aliases=['remove-ban','undo-ban','ub'])
@commands.has_permissions(ban_members = True)
async def unban(ctx,*,member):
    banned_people = await ctx.guild.bans()
    member_name, member_discriminator = member.split('#')
    for no_entry in banned_people:
        user = no_entry.user
...
######################################################################################################
# ECONOMY SYSTEM END
client.run('ODM4OTg2NzY4NjA5OTY4MTI5.YJDFUQ.lx3z_3YiFXffTowK8jjiyFUNpJg')